...
interface DailyReportEditorProps {
  ...
  onTitleChange: (title: string) => void; // 追加
...
const DailyReportEditor: React.FC<DailyReportEditorProps> = ({
  ...
  // 一時的な編集内容を保持するstate
  const [tempContent, setTempContent] = useState(reportContent);
  const [tempTitle, setTempTitle] = useState(reportTitle);

  // props の reportContent が変更されたら tempContent も更新
  useEffect(() => {
    setTempContent(reportContent);
  }, [reportContent]);

  // props の reportTitle が変更されたら tempTitle も更新
  useEffect(() => {
          ...
          <div className="flex items-center space-x-6">
            <div className="font-medium">
              {isEditing ? (
                <input
                  type="text"
                  className="px-2 py-1 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 border border-gray-200"
                  value={tempTitle}
                  onChange={(e) => setTempTitle(e.target.value)}
                  placeholder="日報タイトルを入力"
                />
              ) : (
                <div>{reportTitle || "無題の日報"}</div>
              )}
